define(['./module', 'config'], function (controllers, config) {
    ...
    controllers.controller("projectInstanceController", ['$scope', '$http', '$uibModalInstance', 'toastr', 'projectService', 'metadataService','$location', function ($scope, $http, $uibModalInstance, toastr, projectService, metadataService,$location) {
        var self = this;
        // Event exit popup
        ...
        self.create = function () {
            if ($scope.projectInfo.$valid) {
                $http.post(apiServer + '/apii/projects/', self.project).success(function (data) {
                    console.log(data);
                    toastr.success('Project has been created', 'TMS');
