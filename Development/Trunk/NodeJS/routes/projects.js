...
project_router.get('/:project_id/members/', function (req, res, next) {
    ...
    member_instance.getAllMemberInProject(req.params.project_id, function (data) {
        if (data.length > 0) {
            var min, max;
            min = new Date(data[0].join_date);
            max = new Date(data[0].join_date);
            for (var i = 0; i < data.length; i++) {
                var check = new Date(data[i].join_date); 
                if (check.getTime() > max.getTime()) {
                    max = data[i].join_date;
                }
                if (check.getTime() < min.getTime()) {
                    min = data[i].join_date;
                }
            }
            var a ={max: max, min: min};
            data.push(a);
        }
        res.status(200).json({
            data: data
