...
var assignment_service = require('../services/assignmentService');
...
employees_router.get('/:employee_id/recent_assignments', function(req, res) {
    var get_recent_assignments = new assignment_service();
    get_recent_assignments.retrieveRecentAssignmentAllByUser_Id(req.params.employee_id, function(data) {
        
        res.status(200).json({
            code: 200,
            message: 'Get recent_assignments Successfully',
            data: data
        }).end();
    }, function(err) {
        res.status(200).json({
            code: 404,
            message: 'Get recent_assignments Failed',
            data: err
        }).end();
    });
});

employees_router.put('/:employee_id/recent_assignments/:project_user_id', function(req, res) {
    var update_recent_assignments = new assignment_service();
    update_recent_assignments.updateRecentAssignment(req.body, function(data) {
        res.status(200).json({
            code: 200,
            message: 'update recent_assignments Successfully',
            data: data
        }).end();
    }, function(err) {
        res.status(200).json({
            code: 404,
            message: 'update recent_assignments Failed',
            data: err
        }).end();
    });
});
